...
# --- Sayfa Ayarları ---
st.set_page_config(page_title="Akademik Analiz v43", layout="wide")
st.title("📊 Akademik Karar Destek Sistemi (Veri Garantili)")
st.markdown("""
**Düzeltme 1:** EV/EBITDA için 'Debt' kalemlerini akıllı toplama algoritması.
**Düzeltme 2:** Finviz engeline karşı Yahoo Finance yedekli haber/profil motoru.
""")
...
# --- FİNVİZ & YAHOO HABER SİSTEMİ (Yedekli) ---
@st.cache_data(ttl=1800)
def get_news_profile_robust(ticker):
    """Önce Finviz dener, olmazsa Yahoo'dan çeker."""
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if profile_td:
                data["Profile"] = translate_to_turkish(profile_td.get_text(strip=True))
            # Haberler
    ...
    # 2. Deneme: Yahoo Finance (Eğer Finviz boşsa)
    if not data["News"] or data["Profile"] == "Bulunamadı":
        try:
            ...
            if data["Profile"] == "Bulunamadı":
                ...
                if summary: data["Profile"] = translate_to_turkish(summary)
...
# --- METRİKLER VE TEKNİK (AKILLI BORÇ HESAPLAMA) ---
def fetch_robust_metrics(ticker):
    ...
    try:
        ...
        # 2. EV/EBITDA (ZIRHLI HESAPLAMA v2)
        # Önce Hazır Veri
        ...
        # Manuel Hesaplama (Akıllı Toplama)
        ...
        if mcap:
            ...
            if not bs.empty and not inc.empty:
                ...
                # --- BORÇ BULMA (SMART SUM) ---
                debt = find_value_in_df(curr_bs, ['total', 'debt'])
                if debt is None:
                    # 'Debt' kelimesi geçen tüm satırları bul ve topla (Daha güvenli)
                    debt_sum = 0
                    for idx in curr_bs.index:
                        if 'debt' in str(idx).lower():
                            val = curr_bs[idx]
                            if not pd.isna(val): debt_sum += val
                    debt = debt_sum if debt_sum > 0 else 0
...
# --- FİNVİZ TARAYICI ---
def get_finviz_v43(limit_count, exc, sec, pe, peg, roe_val, de, rsi_val, ma_val):
    filters = []
...
if st.sidebar.button("Analizi Başlat"):
    with st.spinner("Piyasa taranıyor..."):
        df, url = get_finviz_v43(scan_limit, exchange, sector, pe_ratio, peg_ratio, roe, debt_eq, rsi_filter, price_ma)
        ...
        if tik and not hist_long.empty:
            ...
            with tab_news:
                ...
                with st.spinner("Haberler analiz ediliyor..."):
                    ...
                    finviz_data = get_news_profile_robust(tik)
